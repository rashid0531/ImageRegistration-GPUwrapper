...
def register_channels(gpu,impath, outpath, make_rgb=False, crop=False, profile=False, database=False):
    
    #mrc689 for setting specific GPU from command line arguments     
    gw.set_specific_GPU(int(gpu))#
    #gw.get_indexof_GPU()
    ...
    #np.savetxt('output/procssing_time_gpu.csv',indiv_time)
    if profile is True:
...
if __name__ == "__main__":
    ...
    #mrc689 May 17
    ap.add_argument("-g", "--gpu", required=True, help="Path to Select specific GPU.") 
    ...
    try:
        if args["database"] is True:
            ...
            list_of_dicts = register_channels(args["gpu"],args["impath"], outpath=outpath, make_rgb=args["rgb"], crop=args["crop"], profile=args["profile"], database=args["database"])
            ...
            register_channels(args["gpu"],args["impath"], outpath=args["outpath"], make_rgb=args["rgb"], crop=args["crop"], profile=args["profile"], database=args["database"])
